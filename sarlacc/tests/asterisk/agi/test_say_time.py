...
class TestCase(test.TestCase):
    ...
    def test_say_time_failure(self):
        with patch('sys.stdin', StringIO("200 result=-1")
            ...
            self.assertEqual(mocked_out.getvalue(), 'SAY TIME 1363725155 ""\n')
    ...
    def test_say_time_success(self):
        with patch('sys.stdin', StringIO("200 result=0")
            ...
            self.assertEqual(mocked_out.getvalue(), 'SAY TIME 1363725172 ""\n')
            self.assertTrue(res)
            self.assertEqual(dtmf, '')
    ...
    def test_say_time_digit_pressed(self):
        with patch('sys.stdin', StringIO("200 result=49")
            ...
            self.assertEqual(
                mocked_out.getvalue(), 'SAY TIME 1363725192 "1234"\n'
            )
            self.assertTrue(res)
