...
class TestCase(test.TestCase):
    ...
    def test_speech_destory_failure(self):
        with mock.patch(
                'sys.stdout', new_callable=cStringIO.StringIO) as mock_stdout:
            res = self.agi.speech_destroy()
            self.assertEqual(mock_stdout.getvalue(), 'SPEECH DESTROY\n')
            self.assertFalse(res)

    @mock.patch('sys.stdin', cStringIO.StringIO("200 result=1"))
    def test_speech_destory_success(self):
        with mock.patch(
                'sys.stdout', new_callable=cStringIO.StringIO) as mock_stdout:
            res = self.agi.speech_destroy()
            self.assertEqual(mock_stdout.getvalue(), 'SPEECH DESTROY\n')
